use crate::cmd::{get::Get, set::Set, publish::Publish, ping::Ping};
...
impl Client {
    ...
    /// Establish a secure connection with the Redis server located at `addr`.
    pub async fn connect_tls(addr: &str, domain: &str, connector: TlsConnector) -> Result<Client, Error> {
        let socket = TcpStream::connect(addr).await?;

        let domain = ServerName::try_from(domain)
            .map_err(|_| Error::Other("invalid domain name".into()))?
            .to_owned();

        let stream = connector.connect(domain, socket).await.map_err(|e| Error::Other(e.to_string()))?;

        // TlsStream<TcpStream> implements AsyncRead + AsyncWrite + Unpin + Send
        // Connection::new expects S: AsyncStream
    ...
    pub async fn get(&mut self, key: &str) -> Result<Option<Bytes>, Error> {
        ...
        match self.read_response().await? {
            Frame::Simple(value) => Ok(Some(Bytes::from(value))),
